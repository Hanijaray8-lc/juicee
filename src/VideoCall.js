      ...
      try {
        ...
        while (retries > 0 && !currentStream) {
          try {
            ...
            retries--;
            console.warn(`⚠️ Media request failed (retries left: ${retries}):`, err.name, err.message);
            if (retries === 0) {
              // Try fallback constraints if video was requested
              if (type === 'video') {
                try {
                  console.log('🔄 Attempting fallback: basic video without strict resolution');
                  currentStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
                  break;
                } catch (fallbackErr) {
                  console.warn('⚠️ Basic video failed too, falling back to audio-only stream to keep call alive:', fallbackErr.name);
                  try {
                    currentStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
                    setIsCameraOff(true);
                    break;
                  } catch (audioOnlyErr) {
                    throw err; // Re-throw original error if even audio fails
                  }
                }
              } else {
                throw err;
        ...
        socket.once('callAccepted', signal => {
          ...
          try {
            ...
            console.warn('⚠️ Error signaling callAccepted:', sigErr.message);
          }
    ...
    try {
      ...
      while (retries > 0 && !currentStream) {
        try {
          ...
          retries--;
          console.warn(`⚠️ Media request failed in answerCall (retries left: ${retries}):`, err.name, err.message);
          if (retries === 0) {
            if (wantVideo) {
              try {
                console.log('🔄 Attempting basic video fallback in answerCall');
                currentStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
                break;
              } catch (fallbackErr) {
                console.warn('⚠️ Basic video failed, falling back to audio-only stream in answerCall:', fallbackErr.name);
                try {
                  currentStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
                  setIsCameraOff(true);
                  break;
      ...
      try {
        ...
        console.warn('⚠️ Error signaling offer to peer in answerCall:', sigErr.message);
      }
      processPendingCandidates();
