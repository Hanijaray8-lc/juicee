...
const ChatPage = () => {
  ...
  const handleInitiateCall = (targetId, type) => {
    // ⚡ Start pre-warming media immediately on button press (before block checks)
    // This runs async in background — call logic below is unaffected
    if (prewarmCallerMedia) prewarmCallerMedia(type || 'audio');
